...
export async function GET() {
  try {
    ...
    if (columnCheck[0].column_exists === 0) {
      ...
      return NextResponse.json({ success: true, message: "Columna recordings_folder_id agregada correctamente" })
    } else {
      return NextResponse.json({ success: true, message: "La columna recordings_folder_id ya existe" })
    }
  } catch (error) {
    console.error("Error al agregar columna recordings_folder_id:", error)
