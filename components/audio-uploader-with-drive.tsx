...
export function AudioUploaderWithDrive({ onFileSelected, disabled, useGoogleDrive = false }: AudioUploaderProps) {
  ...
  // Modificar la función validateAudioFile para incluir la validación de duración
    ...
    if (file) {
      if (validateAudioFile(file)) {
        ...
        try {
          ...
          if (useGoogleDrive) {
            audioData = await uploadToGoogleDrive(file)
          } else {
            audioData = await processLocalFile(file)
          }
    ...
    <div className="bg-blue-800/30 border border-blue-700/30 rounded-lg p-4 sm:p-6">
      ...
      >
        ...
        <div className="text-xs text-blue-300/70 mb-3 sm:mb-4">
          ...
          {useGoogleDrive && (
            <div className="mt-2 text-blue-200">El archivo se almacenará en tu cuenta de Google Drive</div>
          )}
        </div>
